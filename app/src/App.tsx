...
const angle06FrontBackAssignmentChinese = '角度6前后侧产品分配锁：参考图蓝色文字标注中，画面左下偏前、离镜头更近的鞋是“前侧”，画面右上偏后、被前侧鞋形成前后错落关系的鞋是“后侧”。当上传两张产品鞋图时，第1张产品鞋图必须生成在前侧鞋位置，第2张产品鞋图必须生成在后侧鞋位置；若只上传一张产品鞋图，则前侧和后侧都使用同一张产品图。这个规则只控制产品图分配，不改变角度6原有黄色区域距离、鞋子大小、方向、手持关系和前后错落距离。'

const angle06FrontBackAssignmentEnglish = 'ANGLE-06 FRONT / BACK PRODUCT ASSIGNMENT LOCK: according to the blue labels in the angle-06 reference, the lower-left nearer shoe is the FRONT-SIDE shoe, and the upper-right farther staggered shoe is the BACK-SIDE shoe. When two product shoe references are uploaded, product reference 1 must control ONLY the FRONT-SIDE lower-left/nearer shoe, and product reference 2 must control ONLY the BACK-SIDE upper-right/farther shoe. If only one product shoe reference is uploaded, both front-side and back-side shoes use that same product identity. This assignment controls product identity only; it must not change the existing angle-06 yellow-region distance, shoe size, shoe direction, hand-held relationship, near/far depth, or spacing rules.'

const angle06EqualSharpnessChinese = '角度6双鞋清晰度锁：前侧鞋和后侧鞋都必须一样清晰、一样锐利、一样有产品细节。后侧鞋虽然在前后错落关系里靠后，但不能被景深虚化、不能模糊、不能低细节、不能像背景陪衬；鞋面、鞋底、鞋带/扣件、缝线、材质纹理、边缘轮廓和颜色都要和前侧鞋一样清楚。使用商业产品摄影的深景深或足够清晰的焦平面，让两只鞋同时成为清晰产品主体。'

const angle06EqualSharpnessEnglish = 'ANGLE-06 EQUAL SHARPNESS LOCK: both the front-side shoe and the back-side shoe must be equally sharp, equally detailed, and equally important as product heroes. The back-side upper-right/farther shoe may stay behind in the staggered composition, but it must not be depth-of-field blurred, soft, low-detail, hazy, defocused, or treated as a background prop. Keep shoe upper details, outsole edge, laces/buckles, stitching, material texture, logo/details, color, and contour clarity as crisp as the front-side shoe. Use commercial product photography deep focus or a focus plane that keeps both shoes sharp at the same time.'

const angle06ToeTowardCameraChinese = '角度6鞋头朝镜头锁：两只鞋以近距离手持展示，鞋头朝向画面前方并略微朝下，鞋头更靠近镜头，鞋跟靠近人物身体方向。鞋子呈三分之二正面俯视角，可以同时看到鞋头正面、鞋面、鞋带区域、鞋口内侧和部分侧面厚度；不是纯侧面，也不是完全俯拍平铺。两只鞋整体从左上鞋跟方向延伸到右下鞋头方向，鞋头有明显迎向镜头的透视感，前端略大、后跟略远，保持自然手持展示角度。这个规则只调整鞋头朝向和近镜头透视，不改变原有黄色区域距离、前侧/后侧分配、鞋子大小、手持关系和两鞋清晰度。'

const angle06ToeTowardCameraEnglish = 'ANGLE-06 TOE-TOWARD-CAMERA LOCK: show both shoes as close hand-held display shoes. The toe/front ends point toward the front of the frame and slightly downward, with the toe/front closer to the camera and the heels closer to the model body. Use a three-quarter front high-angle perspective: the viewer can see the toe-front surface, shoe upper, lace area, inner shoe opening, and part of the side thickness at the same time. This is not a pure side view and not a completely flat top-down layout. The pair should visually extend from upper-left heel direction to lower-right toe direction, with obvious toe-toward-camera perspective, slightly larger front ends, and slightly farther heels while keeping a natural hand-held display angle. This rule adjusts only toe orientation and near-camera perspective; it must not change the existing yellow-region distance, front/back assignment, shoe size, hand-held relationship, or equal sharpness.'

const angle06SittingNoFurnitureChinese = '角度6坐姿锁：模特只需要呈现模拟坐姿或侧坐身体关系，身体位于画面左侧局部裁切，用手把两只鞋向镜头展示；不需要出现具体的椅子、沙发、凳子、座椅、靠背、扶手或坐垫等家具。坐姿通过身体倾斜、衣服褶皱、手臂位置和局部腿部位置表达即可，最终背景仍然只按照上传背景图生成，不要为了说明坐姿新增家具或道具。'

const angle06SittingNoFurnitureEnglish = 'ANGLE-06 SITTING POSE WITHOUT FURNITURE LOCK: the model only needs to simulate a seated or side-seated body relationship, with the body cropped on the left side and hands presenting both shoes toward the camera. Do not generate any specific chair, sofa, stool, seat, backrest, armrest, cushion, or seating furniture. Express the seated pose only through body lean, clothing folds, arm position, and a small partial leg area. The final environment must still come only from the uploaded background image; do not add furniture or props to explain the sitting pose.'
...
const mergeAnglePosePrompts: Record<string, string> = {
  ...
  'angle-06': '选定姿势提示：半身近景构图，以鞋子为绝对主体，鞋子的画面占比、大小、覆盖范围和视觉权重必须严格按照角度6参考图里的黄色区域来生成，不要自行放大、缩小或重新估算占比；人物、衣服、腿部和手只作为辅助展示关系。模特模拟坐姿并朝画面左侧形成侧坐身体关系，身体主要位于画面左侧，只截取胸部以下的位置，不出现脸、头部或完整上半身。画面左上方只能看到模特局部衣服和身体轮廓，画面左下角只露出一点点腿部，腿部不是主体。模特双手分别拿着两只鞋向镜头前方展示，手主要抓住鞋子的后脚底部、后跟底部或后跟外侧支撑点。两只鞋子的方向必须一致：鞋后和鞋跟朝向人物身体一侧，鞋头统一朝画面的右下角，两只鞋位于角度6参考图黄色区域对应的位置和范围内，靠近镜头并占据主要视觉中心，鞋身要清晰、完整，形成前后错落的近距离展示关系。手部必须自然真实，按照正常人体手部结构生成；每只可见手保持五指数量正确、指节清晰、关节比例正常、手掌大小合理，手指自然贴合鞋底或后跟受力。不要生成多指、少指、断指、融合手指、漂浮手指、重复手指、过长手指、扭曲手腕、畸形手掌、手指穿模或不合理抓握。' + angle06SittingNoFurnitureChinese + angle06FrontBackAssignmentChinese + angle06EqualSharpnessChinese + angle06ToeTowardCameraChinese,
}
...
const mergeAngleRoleLocks: Record<string, string> = {
  ...
  'angle-06': 'Selected pose prompt role lock: angle-06 must remain a hand-held two-shoe close-up display. Both product shoes are held toward the camera by natural hands. Do not turn either shoe into a worn shoe, do not add feet inside the shoes, do not create a full-body model, and do not change the shoe count. ' + angle06SittingNoFurnitureEnglish + ' ' + angle06FrontBackAssignmentEnglish + ' ' + angle06EqualSharpnessEnglish + ' ' + angle06ToeTowardCameraEnglish,
}
...
function buildAngle06ReferenceLock(angleId: string) {
  ...
  return [
    ...
    angle06SittingNoFurnitureEnglish,
    angle06FrontBackAssignmentEnglish,
    angle06EqualSharpnessEnglish,
    angle06ToeTowardCameraEnglish,
  ].join('\n')
}
...
function buildMergeProductAssignmentPrompt(productCount: number, angleId: string) {
  ...
  if (angleId === 'angle-06') {
    return [
      base,
      'ANGLE-06 TWO PRODUCT SHOE ASSIGNMENT LOCK: angle-06 always shows exactly two hand-held display shoes. When two product shoe references are uploaded, product reference 1 controls the FRONT-SIDE lower-left/nearer shoe, and product reference 2 controls the BACK-SIDE upper-right/farther shoe. If the two references have different colors, styles, materials, or details, the final angle-06 image must show two visibly different shoes in those exact front/back positions.',
      angle06SittingNoFurnitureEnglish,
      angle06FrontBackAssignmentEnglish,
      angle06EqualSharpnessEnglish,
      angle06ToeTowardCameraEnglish,
      ...
      'Final visible check: if two uploaded product references are different but the front-side lower-left/nearer shoe and back-side upper-right/farther shoe look like the same colorway, if product reference 2 does not appear on the back-side shoe, or if the back-side shoe is visibly blurrier/softer/lower-detail than the front-side shoe, the result is wrong.',
    ].join('\n')
  }
  return [
    base,
