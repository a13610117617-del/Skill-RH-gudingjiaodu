...
const mergeAngleRoleLocks: Record<string, string> = {
  ...
  'angle-06': 'Selected pose prompt role lock: angle-06 must remain a hand-held two-shoe close-up display. Both product shoes are held toward the camera by natural hands. Do not turn either shoe into a worn shoe, do not add feet inside the shoes, do not create a full-body model, and do not change the shoe count. ' + angle06FrontBackAssignmentEnglish,
}
...
function buildMergeProductAssignmentPrompt(productCount: number, angleId: string) {
  ...
  if (angleId === 'angle-06') {
    return [
      ...
      'Final visible check: if two uploaded product references are different but the front-side lower-left/nearer shoe and back-side upper-right/farther shoe look like the same colorway, or if product reference 2 does not appear on the back-side shoe, the result is wrong.',
    ].join('\n')
  }
  return [
    base,
