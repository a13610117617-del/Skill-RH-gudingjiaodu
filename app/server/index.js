...
function describeMergeImageUploadedFiles(files = []) {
  ...
  const roleLabels = {
    ...
    background: 'background reference: only source for room, wall, floor, light, and perspective',
...
function buildMergeImageLockInstruction(files = [], angleSource = '') {
  ...
  const backgroundElementLock = 'BACKGROUND LOCK: the uploaded background is the only environment source. Preserve wall, floor, baseboard, space, light direction, color temperature, shadow mood, and every visible background element already present in the uploaded background image. Preserve existing background elements, their relative positions, scale, crop, perspective, and occlusion; do not omit, simplify, replace, or genericize them, and do not invent new background elements. Any ban on extra elements applies only to newly invented elements or non-background references; it must not remove elements already visible in the uploaded background.'
  ...
  return [
    ...
    hasAngleControl
      ? hasModel
        ...
        : `Use image 1 cleaned angle control first and most strongly to determine exact pose/layout, final main shoe count, yellow shoe-region placement/angle/scale, worn-shoe supplements around blue feet, and each blue region classification as LEG/FOOT/ANKLE, HAND/HANDWRIST/FINGER, or minor detail. Use image 2 original angle reference only to verify fine pose and occlusion. Use image 3 background reference for all environment, wall, floor, baseboard, light, and shadows. Use ${productReferenceUseText} from image 4 and later for shoe identity.`
      : hasModel
        ? hasRealPhotoAngle
        ...
        : hasRealPhotoAngle
          ? `Use image 1 real photo angle reference first and most strongly to determine pose/layout, real camera viewpoint, crop, shoe count, shoe placement, body/hand/foot relationship, scale, perspective, and occlusion. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. Use ${productReferenceUseText} from image 3 and later for shoe identity.`
          : `Use image 1 angle mask first and most strongly to determine pose/layout, yellow shoe-region count/placement/angle/scale, and each blue region silhouette as leg/ankle/foot or arm/hand/wrist. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. Use ${productReferenceUseText} from image 3 and later for shoe identity.`,
  ].filter(Boolean).join('\n')
}
function buildMergeImageRolePriorityInstruction() {
  return [
    ...
    'Background image: only source for the final environment, wall, floor, baseboard, space, light direction, atmosphere, shadows, and every visible background element already present in the uploaded background image. Put this environment into all black/background regions. Preserve existing background elements, their relative positions, scale, crop, perspective, and occlusion; do not omit, simplify, replace, or genericize them, and do not invent new background elements. The model-reference background must never appear.',
...
function buildMergeImageRealPhotoAnglePriorityInstruction() {
  return [
    ...
    'Background image: only source for the final environment, wall, floor, baseboard, space, light direction, atmosphere, shadows, and every visible background element already present in the uploaded background image. Preserve existing background elements, their relative positions, scale, crop, perspective, and occlusion; do not omit, simplify, replace, or genericize them, and do not invent new background elements. The model-reference background must never appear.',
...
function buildMergeImageChineseRecordPrompt({ files = [], angleSource = '', requestedSize = '', apiSize = '', hasModelReference = false } = {}) {
  ...
  return [
    ...
    '黑色区域使用上传背景图生成真实背景，包括墙面、地面、踢脚线、空间感、光线方向、色温和阴影氛围。',
    '',
    '优先级：',
...
function buildFixedSingleFootLibraryFinalLock(angleSource = '') {
  ...
  return [
    ...
    '[MUST] BACKGROUND ABSOLUTE LOCK: the uploaded background image is the only environment source and must be followed completely. Preserve the uploaded background wall, floor, baseboard, every visible current-background element, perspective, crop, color temperature, lighting direction, and shadow mood. The final image should look like the fixed leg and uploaded product shoe were placed into the uploaded background, not like the fixed leg photo background.',
    '[MUST NOT] The fixed leg/magenta model reference, angle reference, cleaned control image, product reference, and previous successful examples must never control the final background, wall, floor, props, baseboard, horizon, light pattern, color temperature, shadows, scene layout, white background, room layout, or any non-current-background scene element.',
    ...
    '[MUST] ANGLE-03 / ANGLE-04 PAIR SIZE CONSISTENCY: when angle-03 and angle-04 are generated from different product references, the final shoe outer box, outsole contact line, ankle contact height, leg entry, floor contact shadow size, and camera crop must match across the two outputs. Only shoe appearance, color, material, details, and product identity may differ.',
  ...
  if (referenceFiles.length) {
    ...
    if (imageFiles.length) {
      for (const endpoint of editCandidates) {
        ...
        editLastError = new Error(detail)
...
async function analyzeUploadedMergeAngleReference({ settings, file }) {
  ...
  const instruction = [
    ...
    'For background/environment areas, describe placement, horizon/baseboard/floor-wall relationship if visible, floor contact points, and perspective zones. Background details must come from the uploaded background image during generation.',
...
async function callOpenAIImageWithRetry(args, maxAttempts = 2) {
  let lastError = null
  ...
  try {
    ...
    const finalHardLayoutLock = isFixedSingleFootLibraryAngle03Or04
      ? [
            ...
            'Final pair check for angle-03 and angle-04: compare shoe length, shoe height, outsole baseline, ankle opening height, contact shadow footprint, and camera crop only after both shoes are fitted into their original angle yellow/S regions. Product references may change shoe identity and details only. Never move, enlarge, lower, crop, or recenter the shoe away from the angle yellow/S region to satisfy pair consistency or old size memory.',
          ].join('\n')
